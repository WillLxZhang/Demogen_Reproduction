...
def _get_dataset_lang(dataset_cfg, all_obs_keys):
    """
    Only fall back to a dummy language instruction when the model actually
    consumes language embeddings. This avoids unnecessary CLIP downloads for
    standard non-language training runs.
    """
    if "lang" in dataset_cfg:
        return dataset_cfg["lang"]
    if LangUtils.LANG_EMB_OBS_KEY in all_obs_keys:
        return "dummy"
    return None
...
def train(config, device, resume=False):
    ...
    always_save_latest = config.experiment.save.always_save_latest
    checkpoint_state_mode = config.experiment.save.checkpoint_state_mode
    ...
    for dataset_cfg in config.train.data:
        ...
        dataset_lang = _get_dataset_lang(dataset_cfg=dataset_cfg, all_obs_keys=config.all_obs_keys)
        if dataset_lang is not None:
            env_meta["lang"] = dataset_lang
        ...
        shape_meta_dataset_cfg = dict(dataset_cfg)
        if dataset_lang is not None:
            shape_meta_dataset_cfg["lang"] = dataset_lang
        shape_meta = FileUtils.get_shape_metadata_from_dataset(
            dataset_config=shape_meta_dataset_cfg,
    ...
    epoch_iterator = LogUtils.custom_tqdm(
        range(start_epoch, config.train.num_epochs + 1),
        total=config.train.num_epochs,
    ...
    for epoch in epoch_iterator:
        epoch_iterator.set_description("Epoch {}/{}".format(epoch, config.train.num_epochs))
        ...
        print(json.dumps(train_step_log, sort_keys=True, indent=4))
        ...
        if config.experiment.validate:
            ...
            print(json.dumps(valid_step_log, sort_keys=True, indent=4))
        ...
        if should_save_ckpt:    
            saved_ckpt_path = os.path.join(ckpt_dir, epoch_ckpt_name + ".pth")
            TrainUtils.save_model(
                ...
                obs_normalization_stats=obs_normalization_stats,
                action_normalization_stats=action_normalization_stats,
                model_state_mode=checkpoint_state_mode,
            )

        save_latest_now = always_save_latest or should_save_ckpt or (epoch == config.train.num_epochs)
        if save_latest_now:
            if (saved_ckpt_path is not None) and (not always_save_latest):
                TrainUtils.refresh_checkpoint_links(
                    new_ckpt_path=saved_ckpt_path,
        ...
        if "Loss" in train_step_log:
            epoch_postfix["loss"] = "{:.4f}".format(train_step_log["Loss"])
        if "Optimizer/policy0_lr" in train_step_log:
            epoch_postfix["lr"] = "{:.2e}".format(train_step_log["Optimizer/policy0_lr"])
        epoch_postfix["mem_mb"] = mem_usage
        epoch_iterator.set_postfix(epoch_postfix)
...
def main(args):
    ...
    if args.debug:
        ...
        # preserve any explicit output directory in the config during debug runs.
        # only fall back to /tmp if no output directory is set.
        if config.train.output_dir in (None, ""):
