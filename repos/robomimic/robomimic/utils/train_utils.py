...
def _prepare_serialized_model(model, model_state_mode="full"):
    """
    Build the serialized model payload for checkpointing.

    Args:
        model (Algo instance): model to serialize
        model_state_mode (str): "full" keeps training state, while "policy_only"
            strips optimizer and lr scheduler state to reduce checkpoint size.
    """
    serialized_model = model.serialize()
    if model_state_mode == "full":
        return serialized_model
    if model_state_mode == "policy_only":
        slim_model = {}
        for key in ("nets", "ema"):
            if key in serialized_model:
                slim_model[key] = serialized_model[key]
        return slim_model
    raise ValueError("Unsupported model_state_mode: {}".format(model_state_mode))


def _replace_with_symlink(link_path, target_path):
    """
    Replace @link_path with a symlink that points to @target_path.
    """
...
def refresh_checkpoint_links(new_ckpt_path, latest_model_path, latest_model_backup_path):
    """
    Point last.pth and last_bak.pth to periodic checkpoints without duplicating file contents.
    """
    prev_target = None
    if os.path.lexists(latest_model_path):
        prev_target = os.path.realpath(latest_model_path)
        if os.path.abspath(prev_target) == os.path.abspath(new_ckpt_path):
            prev_target = None

    if prev_target is not None and os.path.exists(prev_target):
        _replace_with_symlink(latest_model_backup_path, prev_target)
        print("\nsaved backup of latest model at {}\n".format(latest_model_backup_path))
    elif os.path.lexists(latest_model_backup_path):
        os.remove(latest_model_backup_path)

    _replace_with_symlink(latest_model_path, new_ckpt_path)
    print("\nsaving latest model at {}...\n".format(latest_model_path))
    print("latest model now points to {}\n".format(new_ckpt_path))


def save_model(model, config, env_meta, shape_meta, ckpt_path, variable_state=None, obs_normalization_stats=None, action_normalization_stats=None, model_state_mode="full"):
    """
    ...
    params = dict(
        model=_prepare_serialized_model(model=model, model_state_mode=model_state_mode),
        config=config.dump(),
