...
from robomimic.scripts.train import train, _get_dataset_lang
...
def test_dataset_lang_resolution():
    try:
        assert _get_dataset_lang({"path": "/tmp/demo.hdf5"}, ["object"]) is None
        assert _get_dataset_lang({"path": "/tmp/demo.hdf5"}, ["lang_emb"]) == "dummy"
        assert _get_dataset_lang(
            {"path": "/tmp/demo.hdf5", "lang": "pick up the cube"},
            ["object"],
        ) == "pick up the cube"
        ret = colored("passed!", "green")
    except Exception as e:
    ...
    test_name = "dataset-lang-resolution"
    print("{}: {}".format(test_name, ret))
