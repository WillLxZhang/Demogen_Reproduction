...
from demo_generation.handlepress_robosuite_wrapper import (
...
class HandlePressPhaseCopyReplayConsistentDemoGen(LiftPhaseCopyExecConsistentDemoGen):
    def __init__(self, cfg):
        ...
        if source_demo_hdf5 is None:
            raise ValueError(
                "HandlePressPhaseCopyReplayConsistentDemoGen requires cfg.source_demo_hdf5"
            )
        ...
        if env_name != "HandlePress":
            raise ValueError(
                "HandlePressPhaseCopyReplayConsistentDemoGen expects "
                f"HandlePress source demos, got env={env_name}"
            )

        cprint(
            (
                "HandlePress replay-consistent fork: "
    ...
    def one_stage_augment(self, n_demos, render_video=False, gen_mode="random"):
        ...
        cprint(
            (
                "HandlePress replay-consistent schedule: "
        ...
        env = HandlePressRobosuite3DEnv(str(self.source_demo_hdf5), render=False)
            ...
            for i in tqdm(range(self.n_source_episodes)):
                ...
                if skill_1_frame <= 0:
                    raise ValueError(
                        ...
                        "to a copy of the source trajectory."
                    )
