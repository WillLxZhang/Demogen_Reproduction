...
class TrainDiffusionUnetHybridPointcloudWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        should_checkpoint = (
            cfg.checkpoint.save_ckpt
            or cfg.checkpoint.save_last_ckpt
            or cfg.checkpoint.save_last_snapshot
        )
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in tqdm.tqdm(range(cfg.training.num_epochs)):
                ...
                # checkpoint
                if checkpoint_every is not None and (self.epoch % checkpoint_every) == (checkpoint_every - 1):
    ...
    def eval(self):
        ...
        ckpt_tag = getattr(cfg.eval, "checkpoint_tag", "latest")
        lastest_ckpt_path = self.get_checkpoint_path(tag=ckpt_tag)
        if lastest_ckpt_path.is_file():
