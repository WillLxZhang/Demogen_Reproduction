...
class pcd_visualizer:
    ...
    @staticmethod
    def preview_in_open3d(point_clouds):
        """
        Exports a video from a series of point clouds.

        Args:
            point_clouds (list of np.ndarray): A list of point clouds, each with shape [N, 6] containing XYZRGB channels.

        Returns:
            None
        """
        vis = o3d.visualization.pcd_visualizer()
        vis.create_window()
        ...
        for pcd in range(point_clouds):
            # now modify the points of your geometry
            # you can use whatever method suits you best, this is just an example
            geometry.points = pcd[:, :3]
            geometry.colors = pcd[:, 3:6] / 255.0
