...
from replay_zarr_episode import TASK_OBJECT_STATE_INDICES, ensure_parent, load_env_name, load_reset_state


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Replay one episode from an exported robomimic-style low_dim HDF5 in robosuite."
    )
...
def parse_translation_attr(raw) -> np.ndarray | None:
    ...
    if arr.shape != (3,):
        raise ValueError(f"object_translation attr must have shape (3,), got {arr.shape}")
    return arr
...
def load_lowdim_episode(dataset_path: Path, demo_key: str | None, episode: int | None):
    with h5py.File(dataset_path, "r") as f:
        ...
        object_translation = parse_translation_attr(ep.attrs.get("object_translation", None))
...
def main() -> None:
    ...
    if object_translation is not None and object_state_indices is not None:
