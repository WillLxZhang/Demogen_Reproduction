...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Continuously evaluate new robomimic checkpoints as they appear during training, "
    ...
    parser.add_argument(
        "--cpu-only",
        action="store_true",
        help="Force evaluation onto CPU to avoid GPU contention with training.",
    )
    parser.add_argument(
        "--nice",
        type=int,
    ...
    parser.add_argument(
        "--exit-when-done",
        action="store_true",
        help="Exit after training finishes and all checkpoints seen so far have been evaluated.",
    )
...
def main() -> int:
    ...
    while not stop:
        ...
        if args.exit_when_done and training_finished(train_log_path):
            print(f"[{now_str()}] training finished and no pending checkpoints, exiting", flush=True)
            break
