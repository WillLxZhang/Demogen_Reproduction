...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate one HandlePress robomimic checkpoint on multiple seeds and summarize results."
    )
    ...
    parser.add_argument("--seeds", type=int, nargs="+", required=True, help="Seeds to evaluate")
...
def build_command(repo_root: Path, checkpoint: Path, seed: int, n_rollouts: int, horizon: int, conda_env: str) -> list[str]:
    return [
        "conda",
        "run",
        "-n",
        conda_env,
        "python",
        str(repo_root / "forks" / "handlepress" / "run_trained_agent_handlepress.py"),
        "--agent",
        str(checkpoint),
        "--n_rollouts",
        str(n_rollouts),
    ...
    while pending or running:
        while pending and len(running) < max(1, int(args.parallel_jobs)):
            ...
            proc = subprocess.Popen(
                build_command(repo_root, checkpoint, seed, args.n_rollouts, args.horizon, args.conda_env),
                cwd=repo_root,
                env=env,
                stdout=log_file,
